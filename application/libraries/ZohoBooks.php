...
class ZohoBooks
{
	 ...
	 *
	 */
	public function getOrganization($authToken)
	{
		//print_r($this->$organizationUrl);

		//echo $this->organizationUrl;
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $this->apiUrl . $this->organizationUrl . '?authtoken=' . $authToken);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_TIMEOUT, $this->timeout);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
