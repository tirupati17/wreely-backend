 ...
 */
 ...
 /*
 * ------------------------WREELY RECEPTION PLATFORM START ------------------------
 */
$vendor_id = NULL;
function authenticate(\Slim\Route $route) {
    ...
    if ($access_token != NULL) {
        ...
        if (!$db->isValidAccessToken($access_token)) {
            $response["error"] = true;
            ...
            global $vendor_id;
            $vendor_id = $db->getUserId($access_token);
        }
    } else {
        $response["error"] = $access_token;
        $response["message"] = "Access token is misssing";
...
$app->post('/user/login', function() use ($app) {
    ...
    verifyRequiredJsonFields($arrayName = array('email' => $email, 'password' => $password));
    $response = array();
...
$app->post('/enquiries', 'authenticate', function() use ($app) {
    //check and parse valid json
    $bodyData = isValidJson($app->request->getBody());

    // reading valid post params from json otherwise return empty string
    $full_name                       = isset($bodyData->full_name) ? $bodyData->full_name : "";
    $email_id                        = isset($bodyData->email_id) ? $bodyData->email_id : "";
    $contact_no                      = isset($bodyData->contact_no) ? $bodyData->contact_no : "";
    $occupation                      = isset($bodyData->occupation) ? $bodyData->occupation : "";
    $reason                          = isset($bodyData->reason) ? $bodyData->reason : "";
    $reference_source                = isset($bodyData->reference_source) ? $bodyData->reference_source : "";
    $how_many_people                 = isset($bodyData->how_many_people) ? $bodyData->how_many_people : "";
    $tell_us_more                    = isset($bodyData->tell_us_more) ? $bodyData->tell_us_more : "";

    //check for required parameter
    verifyRequiredJsonFields($arrayName = array('full_name' => $full_name, 'email_id' => $email_id, 'contact_no' => $contact_no, 'reason' => $reason));
    $response = array();

    global $vendor_id;
    $db = new DbHandler();
    $enquiry_id = $db->createEnquiry($full_name, $email_id , $contact_no , $occupation , $reason , $reference_source, $how_many_people, $tell_us_more, $vendor_id);
    $vendor_data = $db->getUserDetailByUserID($vendor_id);

    if ($enquiry_id != NULL) {
        sendEnquiryMailViaPepipost($full_name, $contact_no, $email_id, $reference_source, $reason, $how_many_people, $tell_us_more, $vendor_data);
...
$app->post('/sendAttendanceReportToMember', 'authenticate', function() use ($app) {
    //check and parse valid json
    $bodyData = isValidJson($app->request->getBody());

    // reading valid post params from json otherwise return empty string
    $member_name                       = isset($bodyData->member_name) ? $bodyData->member_name : "";
    $start_date                        = isset($bodyData->start_date) ? $bodyData->start_date : "";
    $end_date                          = isset($bodyData->end_date) ? $bodyData->end_date : "";
    $vendor_name                       = isset($bodyData->vendor_name) ? $bodyData->vendor_name : "";
    $company_name                      = isset($bodyData->company_name) ? $bodyData->company_name : "";
    $receiver_email                    = isset($bodyData->receiver_email) ? $bodyData->receiver_email : "";
    $attachment_encoded                = isset($bodyData->attachment_encoded) ? $bodyData->attachment_encoded : "";

    //check for required parameter
    verifyRequiredJsonFields($arrayName = array('receiver_email' => $receiver_email, 'attachment_encoded' => $attachment_encoded, 'member_name' => $member_name, 'start_date' => $start_date, 'vendor_name' => $vendor_name, 'company_name' => $company_name));
    $response = array();
    ...
    if (sendAttendanceReportToMember($member_name, $start_date, $end_date, $company_name, $receiver_email, $attachment_encoded, $vendor_data)) {
...
$app->put('/enquiries', 'authenticate', function() use ($app) {
    //check and parse valid json
    $bodyData = isValidJson($app->request->getBody());
    ...
    //check for required parameter
    verifyRequiredJsonFields($arrayName = array('headline' => $headline, 'summary' => $summary, 'availability_type_id' => $availability_type_id, 'stylist_profile_picture_url' => $stylist_profile_picture_url, 'stylist_experiance_type_id' => $stylist_experiance_type_id, 'price_type' => $price_type));
    $response = array();

    global $vendor_id;
    $db = new DbHandler();
    $enquiry_id = $db->updateEnquiry($user_id, $full_name, $email_id, $contact_no, $occupation, $reason, $reference_source);
...
$app->delete('/enquiries/:enquiry_id', 'authenticate', function($enquiry_id) use ($app) {
    //check and parse valid json
    $bodyData = isValidJson($app->request->getBody());

    // reading valid post params from json otherwise return empty string
    $enquiry_id = isset($bodyData->enquiry_id) ? $bodyData->enquiry_id : "";

    //check for required parameter
    verifyRequiredJsonFields($arrayName = array('enquiry_id' => $enquiry_id));
    $response = array();

    global $vendor_id;
    $db = new DbHandler();
    $result = $db->deleteEnquiry($enquiry_id);
...
$app->get('/enquiries', 'authenticate', function() use ($app) {
    // reading valid post params from json otherwise return empty string
    $enquiry_id = $app->request->get('enquiry_id');
    $response = array();

    global $vendor_id;
    $db = new DbHandler();
    $result = $db->getEnquiry($enquiry_id);

    if ($result != NULL) {
        $response["error"] = false;
        $response["enquiries"] = $result;
        echoResponse(200, $response);
    } else {
        $response["error"] = $enquiry_id;
        $response["message"] = "Oop's something went wrong.";
        echoResponse(400, $response);
    }
});

$app->post('/flexiattendance', 'authenticate', function() use ($app) {
    //check and parse valid json
    $bodyData = isValidJson($app->request->getBody());

    // reading valid post params from json otherwise return empty string
    $member_id                       = isset($bodyData->member_id) ? $bodyData->member_id : "";
    $signature_base_64               = isset($bodyData->signature_base_64) ? $bodyData->signature_base_64 : "";
    $company_id                      = isset($bodyData->company_id) ? $bodyData->company_id : "";
    $membership_type_id              = isset($bodyData->membership_type_id) ? $bodyData->membership_type_id : "";

    //check for required parameter
    //verifyRequiredJsonFields($arrayName = array('member_id' => $member_id, 'attendance_date' => $attendance_date, 'signature_base_64' => $signature_base_64));
    $response = array();

    global $vendor_id;
    $db = new DbHandler();
    $enquiry_id = $db->createFlexiAttendance($member_id, $signature_base_64, $company_id, $membership_type_id, $vendor_id);
    $vendor_data = $db->getUserDetailByUserID($vendor_id);

    if ($enquiry_id != NULL) {
        $data = $db->getMemberFullDetailsWithMemberId($member_id);
        sendDailyAttendanceStatusToMember($data['member_name'], $data['vendor_name'],  $data['attendance_date'], $data['member_email_id'], $data['no_of_days_left'], $vendor_data);

        $response["error"] = false;
        $response["message"] = "Successfully created.";
        echoResponse(201, $response);
    } else {
        $response["message"] = "Oop's something went wrong. Please try again.";
        echoResponse(400, $response);
    }
});

$app->post('/deliveryservices', 'authenticate', function() use ($app) {
    //check and parse valid json
    $bodyData = isValidJson($app->request->getBody());

    // reading valid post params from json otherwise return empty string
    $member_id                       = isset($bodyData->member_id) ? $bodyData->member_id : "";
    $from                            = isset($bodyData->from) ? $bodyData->from : "";
    ...
    global $vendor_id;
    $db = new DbHandler();
    $memberData = $db->getMemberWithMemberId($member_id);
    $countryCode = "+91"; //fetch this from db
    $mobileNumber = $memberData["contact_no"];
    $memberName = $memberData["name"];
    $emailId = $memberData["emailId"];
    $vendorName = "Wreely";

    $message = urlencode("Hello ".$memberName." ".""
            . "We have received your courier today from ".$from.". "
            . "Please check and confirm at Reception. Thanks!! "
            . "Regards, "
            . $vendorName." Reception");

    $url = "http://www.softsms.in/app/smsapi/index.php?key=5853827f7db8b&type=text&contacts=".$mobileNumber."&senderid=AARGRP&msg=".$message;
    if (fetchData($url)) {
        $response["error"] = false;
        $response["message"] = "Successfully notified.";
        echoResponse(200, $response);
    } else {
        $response["message"] = "Oop's something went wrong.";
        echoResponse(400, $response);
...
$app->get('/companies/:company_id', 'authenticate', function($company_id) use ($app) {
            global $vendor_id;
            $db = new DbHandler();
            $companies = $db->getCompanyDetailsWithId($company_id);

            $response["companies"] = $companies ? $companies : array();
            echoResponse(200, $response);
});

$app->get('/members', 'authenticate', function() use ($app) {
            global $vendor_id;
            $db = new DbHandler();
            $members = $db->getMembers($vendor_id);

            $response["members"] = $members ? $members : array();
            echoResponse(200, $response);
});

$app->get('/members/:member_id', 'authenticate', function($member_id) use ($app) {
            global $vendor_id;
            $db = new DbHandler();
            $members = $db->getMemberDetailsWithId($member_id);

            $response["members"] = $members ? $members : array();
            echoResponse(200, $response);
});

$app->get('/:company_id/members', 'authenticate', function($company_id) use ($app) {
            global $vendor_id;
            $db = new DbHandler();
            $members = $db->getCompanyMembers($company_id);
...
$member_id = NULL;
function authenticateMember(\Slim\Route $route) {
    // Getting request headers
    $headers = apache_request_headers();
    $response = array();
    $app = \Slim\Slim::getInstance();
...
$app->post('/member/login/confirmation', function() use ($app) {
    ...
    $member = $db->confirmLoginCodeInMember($member_id, $login_code);
...
$app->get('/meetingrooms', 'authenticate', function() use ($app) {
    global $vendor_id;
    $db = new DbHandler();
...
$app->get('/meetingroom/bookings/room_id=:room_id&start_date=:start_date&end_date=:end_date', 'authenticate', function($room_id, $start_date, $end_date) use ($app) {
...
$app->get('/meetingroom/bookings/cancel/booking_id=:booking_id&member_id=:member_id', 'authenticate', function($booking_id, $member_id) use ($app) {
...
$app->post('/meetingroom/bookings', 'authenticate', function() use ($app) {
    ...
    //check for required parameter
    verifyRequiredJsonFields($arrayName = array('member_id' => $member_id, 'room_id' => $room_id, 'start_time' => $start_time, 'end_time' => $end_time));
...
$app->get('/meetingroom/bookings/history/member_id=:member_id&start_date=:start_date&end_date=:end_date', 'authenticate', function($member_id, $start_date, $end_date) use ($app) {
...
$app->get('/events/per_page=:per_page&page_number=:page_number', 'authenticate', function($per_page, $page_number) use ($app) {
    global $vendor_id;
    $db = new DbHandler();
    $result = $db->getVendorEvents($vendor_id, $per_page, $page_number);

    $response["events"] = $result ? $result : array();
    echoResponse(200, $response);
});

$app->get('/events/attend/event_id=:event_id&member_id=:member_id', 'authenticate', function($event_id, $member_id) use ($app) { //should use delete and solve access token problem
    $db = new DbHandler();
...
$app->get('/events/unattend/event_id=:event_id&member_id=:member_id', 'authenticate', function($event_id) use ($app) { //should use delete and solve access token problem
    global $memberId;
    $db = new DbHandler();
    $result = $db->unAttendEvent($event_id, $memberId);
...
$app->post('/memberwalls', 'authenticate', function() use ($app) { //for edit change post to put
    ...
    //check for required parameter
    verifyRequiredJsonFields($arrayName = array('image_url' => $image_url, 'post_description' => $post_description, 'post_by_member_id' => $post_by_member_id));
...
$app->get('/memberwalls/per_page=:per_page&page_number=:page_number', 'authenticate', function($per_page, $page_number) use ($app) {
    global $vendor_id;
    $db = new DbHandler();
    $result = $db->getMemberWallPost($vendor_id, $per_page, $page_number);

    $response["member_walls"] = $result ? $result : array();
    echoResponse(200, $response);
});

$app->get('/memberwalls/delete/post_id=:post_id&post_by_member_id=:post_by_member_id', 'authenticate', function($post_id, $post_by_member_id) use ($app) { //should use delete and solve access token problem
    global $vendor_id;
    $db = new DbHandler();
    $result = $db->deleteMemberWallPost($post_id, $post_by_member_id);

    $response["error"] = false;
    echoResponse(200, $response);
});

$app->get('/memberwalls/likes/post_id=:post_id&per_page=:per_page&page_number=:page_number', 'authenticate', function($post_id, $per_page, $page_number) use ($app) {
    global $vendor_id;
    $db = new DbHandler();
...
$app->get('/memberwalls/comments/post_id=:post_id&per_page=:per_page&page_number=:page_number', 'authenticate', function($post_id, $per_page, $page_number) use ($app) {
    global $vendor_id;
    $db = new DbHandler();
...
$app->post('/memberwalls/likes', 'authenticate', function() use ($app) { //for edit change post to put
    ...
    $like_by_member_id = isset($bodyData->like_by_member_id) ? $bodyData->like_by_member_id : "";

    //check for required parameter
    verifyRequiredJsonFields($arrayName = array('post_id' => $post_id, 'like_by_member_id' => $like_by_member_id));

    global $vendor_id;
    $db = new DbHandler();
    $result = $db->submitLikeForPost($post_id, $like_by_member_id);
    ...
    if ($result == 0) {
        ...
        $response["error"] = false;
        $response["member_wall_likes"] = array($result);
        echoResponse(200, $response);
    }
});

$app->post('/memberwalls/comments', 'authenticate', function() use ($app) { //for edit change post to put
    ...
    //read valid post params from json otherwise return empty string
    $post_id = isset($bodyData->post_id) ? $bodyData->post_id : "";
    $comment_by_member_id = isset($bodyData->comment_by_member_id) ? $bodyData->comment_by_member_id : "";
    $comment = isset($bodyData->comment) ? $bodyData->comment : "";

    //check for required parameter
    verifyRequiredJsonFields($arrayName = array('post_id' => $post_id, 'comment_by_member_id' => $comment_by_member_id, 'comment' => $comment));

    global $vendor_id;
    $db = new DbHandler();
    $result = $db->submitCommentForPost($post_id, $comment_by_member_id, $comment);
...
function sendViaMsg91($countryCode, $mobileNumber, $message) {
    ...
    $auth_key = "350819Abtsifq5d45fef4c3eP1";
